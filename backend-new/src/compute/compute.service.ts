...
export class ComputeService {
  ...
  private async callOrchestration(
    ...
    if (!baseUrl) {
      throw new ServiceUnavailableException(
        'Session orchestration URL not configured for node',
      );
    }
      ...
      const orchResponse = (await this.callOrchestration(
        ...
        'POST',
        {
          ...
          ...(storageTransport === 'nvmeof_tcp' && storageVolume?.node && {
            ...
            // Pass storage provision secret for orchestrator to call storage-provision unmount
            storage_provision_secret: process.env.USER_STORAGE_PROVISION_SECRET,
          }),
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (eventsData.overallStatus === 'ready' && eventsData.connectionInfo) {
          ...
          const nodeIp = node.ipCompute;
          if (!nodeIp) {
            throw new Error(`Compute IP missing for node ${node.hostname}`);
          }
  ...
  async terminateSession(
    ...
    if (result.containerName) {
      ...
      // Capture vol data for use in remount logic below (vol is scoped to this block)
      let volStorageUid: string | undefined;
      let volNodeIpStorage: string | undefined;
      ...
      if (sess?.storageTransport && sess.storageNodeId) {
        try {
          const vol = await this.prisma.userStorageVolume.findFirst({
            ...
            include: { node: { select: { ipManagement: true, ipCompute: true, ipStorage: true } } },
          });
          if (vol) {
            volStorageUid = vol.storageUid;
            volNodeIpStorage = vol.node?.ipStorage ?? undefined;
            const volNodeIpManagement = vol.node?.ipManagement || vol.node?.ipCompute || undefined;
            cleanupPayload.storage_uid = volStorageUid;
            cleanupPayload.storage_backend = vol.storageBackend ?? 'zfs_dataset';
            // Pass storage node IP so orchestrator can call storage-provision for unmount
            if (volNodeIpStorage) {
              cleanupPayload.storage_node_ip = volNodeIpStorage;
            }
            if (sess.storageTransport === 'nvmeof_tcp') {
              cleanupPayload.nvme_subsystem = `laas-${volStorageUid}`;
            }
          }
        } catch (e) {
      ...
      try {
        ...
        // After cross-node NVMe-oF session ends, remount zvol on storage node for File Store access
        // This allows the File Store UI to see user files after the compute session ends
        if (
          sess?.storageTransport === 'nvmeof_tcp' &&
          volStorageUid &&
          volNodeIpStorage
        ) {
          // Find the node's ipManagement to make the HTTP request (backend -> storage node via Tailscale)
          const volNode = await this.prisma.node.findUnique({
            where: { id: sess.storageNodeId! },
            select: { ipManagement: true, ipCompute: true }
          });

          const remountIp = volNode?.ipManagement || volNode?.ipCompute;

          if (remountIp) {
            try {
              this.logger.log(
                `Requesting storage node ${remountIp} to remount zvol for ${volStorageUid}`,
              );
              // Call storage-provision on the storage node to remount the zvol
              await this.storageService.remountZvolOnNode(remountIp, volStorageUid);
              this.logger.log(`Storage node remount completed for ${volStorageUid}`);
            } catch (remountErr) {
              // Log but don't fail the session termination
              this.logger.warn(
                `Failed to remount zvol on storage node for ${volStorageUid}: ${remountErr}`,
              );
            }
          }
        }
